...
def _quick_predict(code: str, is_index: bool = False) -> dict:
    ...
    result = {
        ...
        "bullish_count": up_signals,
        "bearish_count": down_signals,
        ...
        "plus_di": float(latest.get("dmi_plus_di", 0) or 0),
        "minus_di": float(latest.get("dmi_minus_di", 0) or 0),
        "ma_arrangement": "bullish" if ma5 and ma10 and ma20 and not pd.isna(ma5) and not pd.isna(ma10) and not pd.isna(ma20) and ma5 > ma10 > ma20 else ("bearish" if ma5 and ma10 and ma20 and not pd.isna(ma5) and not pd.isna(ma10) and not pd.isna(ma20) and ma5 < ma10 < ma20 else "neutral"),
        "alpha": 0,
        "beta": 1.0,
        "market_regime": "unknown",
        "strategy_bullish_votes": 0,
        "strategy_bearish_votes": 0,
        "support": float(latest.get("box_bottom", 0) or 0),
        "resistance": float(latest.get("box_top", 0) or 0),
    }
