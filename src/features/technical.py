...
class TechnicalFeatures(BaseFeatureExtractor):
    ...
    def extract(self, stock_code: str, **kwargs) -> pd.DataFrame:
        ...
        df["golden_cross_5_10"] = (ma5_above_ma10_prev == False) & (
            ma5_above_ma10_now == True
        )
        df["golden_cross_5_10"] = df["golden_cross_5_10"].astype(int)
        df["death_cross_5_10"] = (ma5_above_ma10_prev == True) & (
            ma5_above_ma10_now == False
        )
        ...
        df["golden_cross_10_20"] = (ma10_above_ma20_prev == False) & (
            ma10_above_ma20_now == True
        )
        df["golden_cross_10_20"] = df["golden_cross_10_20"].astype(int)
        df["death_cross_10_20"] = (ma10_above_ma20_prev == True) & (
            ma10_above_ma20_now == False
        )
        ...
        df["golden_cross_5_20"] = (ma5_above_ma20_prev == False) & (
            ma5_above_ma20_now == True
        )
        df["golden_cross_5_20"] = df["golden_cross_5_20"].astype(int)
        df["death_cross_5_20"] = (ma5_above_ma20_prev == True) & (
            ma5_above_ma20_now == False
        )
        ...
        df["pattern_vol_surge_rise"] = (
            (vol_now / vol_10d_ago > 2.0) & (ret_10d > 0)
        ).astype(int)
        ...
        df["ma_slope_20"] = (
            df["ma_20"].pct_change(5) / 5
        )  # 5-day rate of change per day

        # ========== Deviation features (乖离率) ==========
        # Absolute deviation from moving averages
        df["deviation_ma5_abs"] = np.abs((df["close"] - df["ma_5"]) / df["ma_5"] * 100)
        df["deviation_ma10_abs"] = np.abs(
            (df["close"] - df["ma_10"]) / df["ma_10"] * 100
        )
        df["deviation_ma20_abs"] = np.abs(
            (df["close"] - df["ma_20"]) / df["ma_20"] * 100
        )
        ...
        df["price_stabilize"] = (df["is_bullish"] == 1) & (
            df["close"] > df["low_20d"]
        ).astype(int)
        ...
        ).astype(float)  # 0=low, 1=medium, 2=high
        ...
        # Winsorize extreme values to handle outliers
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()

        # Skip certain columns that shouldn't be winsorized
        skip_cols = [
            "date",
            "stock_code",
            "close",
        ...
        for col in numeric_cols:
            ...
            if col in df.columns:
                # Calculate percentile bounds (1st and 99th percentile)
                lower = df[col].quantile(0.01)
                upper = df[col].quantile(0.99)
                # Winsorize values outside bounds
        ...
        # Final fill with 0 for any remaining NaN
        df = df.fillna(0)
