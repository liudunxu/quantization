...
class MoneyFlowFeatures(BaseFeatureExtractor):
    ...
    def _extract_a_share_money_flow(self, stock_code: str, days: int) -> pd.DataFrame:
        ...
        try:
            ...
            result['main_net_flow_ratio'] = mf_df['主力净流入-净占比'] / 100  # Convert to ratio
            ...
            result['main_net_flow_ratio_ma5'] = result['main_net_flow_ratio'].rolling(window=5).mean()
            ...
            result['main_net_flow_volatility'] = result['main_net_flow'].rolling(window=10).std()
            ...
            import logging
            logging.warning(f"[MoneyFlowFeatures] Failed to fetch money flow for {stock_code}: {e}")
            return pd.DataFrame()
