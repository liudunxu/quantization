...
class MarketFeatures(BaseFeatureExtractor):
    ...
    def extract(self, stock_code: str, **kwargs) -> pd.DataFrame:
        ...
        for period in [5, 10, 20]:
            ...
            df[f"index_ma_{period}_ratio"] = (
                df["index_close"] / df[f"index_ma_{period}"]
            )
        ...
        # Index momentum
        for period in [5, 10, 20]:
            df[f"index_momentum_{period}"] = (
                df["index_close"] / df["index_close"].shift(period) - 1
            )
        ...
        if len(df) >= 252:
            ...
            df["index_position_52w"] = (df["index_close"] - df["index_low_52w"]) / (
                df["index_high_52w"] - df["index_low_52w"] + 1e-10
            )
        else:
            # Use available data for position calculation
            df["index_high_52w"] = (
                df["index_close"].rolling(window=min(60, len(df))).max()
            )
            df["index_low_52w"] = (
                df["index_close"].rolling(window=min(60, len(df))).min()
            )
            df["index_position_52w"] = (df["index_close"] - df["index_low_52w"]) / (
                df["index_high_52w"] - df["index_low_52w"] + 1e-10
            )
